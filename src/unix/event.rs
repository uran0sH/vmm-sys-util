...
pub fn new_event_notifier_and_consumer(
    ...
    if flag.contains(EventFlag::CLOEXEC) {
        // SAFETY: Safe because we check the fd is valid and check the return value.
        let flags = unsafe { libc::fcntl(eventfd_clone.as_raw_fd(), libc::F_GETFD) };
        if flags < 0 {
            return Err(io::Error::last_os_error());
        }
        // SAFETY: Safe because we check the fd is valid and check the return value.
...
mod tests {
    ...
    fn test_new_event_notifier_and_consumer() {
        let (notifier, consumer) = new_event_notifier_and_consumer(EventFlag::empty())
            .expect("Failed to create notifier and consumer");
        notifier.notify().unwrap();
        assert!(consumer.consume().is_ok());
    }
    ...
    fn test_read_nothing() {
        let (_notifier, consumer) = new_event_notifier_and_consumer(EventFlag::NONBLOCK)
            .expect("Failed to create notifier and consumer");
    ...
    #[test]
    fn test_cloexec() {
        let (_notifier, _consumer) = new_event_notifier_and_consumer(EventFlag::CLOEXEC)
            .expect("Failed to create notifier and consumer");
        ...
        unsafe {
            ...
            if pid == 0 {
                ...
                let arg1 = CString::new("10").expect("CString::new failed");
                let ret = libc::execl(
                    path.as_ptr(),
                    arg0.as_ptr(),
                    arg1.as_ptr(),
